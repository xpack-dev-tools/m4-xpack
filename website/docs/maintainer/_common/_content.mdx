...
**@xpack-dev-tools/m4** is an xpm package that packages platform-specific archives, built with the [xPack Build Box (XBB)](https://xpack.github.io/xpack-build-box/).
...
The builds currently execute on 4 dedicated machines (x64 macOS, arm64 macOS, x64 GNU/Linux, arm64 GNU/Linux).
...
```sh
rm -rf ~/Work/xpack-dev-tools/m4-xpack.git/package-lock.json
rm -rf ~/Work/xpack-dev-tools/m4-xpack.git/build-assets/package-lock.json
...
(such as **1.4.20-1.1**) with the following expression:
...
runner, the multi-version macOS tests execute on Travis.

To trigger the Travis test, execute the **trigger-travis-macos** xpm action:
...
- In `CHANGELOG.md`, add the release date and a message
  such as _**\* v1.4.20-1 released**_
...
tagged such as **v1.4.20-1** (mind the dash in the middle!) and
named such as **xPack GNU M4 v1.4.20-1** (mind the dash),
...
- **Commit** all changes with the message
  _**package.json: update URLs for 1.4.20-1.1 release**_
...
- Navigate to the [GitHub Releases](https://github.com/xpack-dev-tools/m4-xpack/releases) page
