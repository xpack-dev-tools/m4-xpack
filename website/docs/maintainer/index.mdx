...
ssh sessions to the build machines (`xbbli`, `xbbla`, `xbbla32`, `xbbmi`, `xbbma`):
...
named like **xPack GNU M4 v{customField('xpackVersion')}** (mind the dash),
...
<CodeBlock language="sh"> {
`npm dist-tag add @xpack-dev-tools/m4@${customField('xpackVersion')}.${customField('npmSubversion')} latest
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`npm unpublish @xpack-dev-tools/m4@${customField('xpackVersion')}.${customField('npmSubversion')}
`} </CodeBlock>
...
- paste the release name like _**xPack GNU M4 v{customField('xpackVersion')} released**_
