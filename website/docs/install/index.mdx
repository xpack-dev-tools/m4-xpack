...
[![npm (scoped)](https://img.shields.io/npm/v/%40xpack-dev-tools%2Fm4?label=xpm&color=blue)](https://www.npmjs.com/package/@xpack-dev-tools/m4/)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/xpack-dev-tools/m4-xpack/total)
[![npm](https://img.shields.io/npm/dm/%40xpack-dev-tools%2Fm4?label=xpm%20downloads)](https://www.npmjs.com/package/@xpack-dev-tools/m4/)

The **xPack GNU M4** can be installed automatically, using the `xpm` command
(the recommended method), or manually, by downloading and unpacking one of the
platform specific archives.

## Automated install

The easiest (and recommended) way to install GNU M4
is via **xpm** and the package available
as <a href={`https://www.npmjs.com/package/@xpack-dev-tools/m4`}><code>@xpack-dev-tools/m4</code></a> from
...
<code>xpacks/@xpack-dev-tools/m4</code>.
...
**m4** package into the project.

The command to install the **latest** available version of **m4** is:
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/m4@${customField('releaseVersion')} --verbose
`} </CodeBlock>
...
* identify the platform specific archive for the latest available version
  of GNU M4, download it into a cache and unpack
...
* create a local symbolic link
  like <code>xpacks/@xpack-dev-tools/m4</code> pointing to the
...
* add links/forwarders into
  ...
  <code>xpacks/@xpack-dev-tools/m4/.content/bin</code>;
* add <code>@xpack-dev-tools/m4</code> to
  `package.json` as a
  development dependency; this associates a specific version of GNU M4 with
...
it is also possible to install GNU M4 only globally in
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/m4@latest --global --verbose
`} </CodeBlock>
...
In order to access the GNU M4 binaries installed in
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/m4/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/m4/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`xpm uninstall @xpack-dev-tools/m4 --verbose
`} </CodeBlock>
...
the <a href={`https://github.com/xpack-dev-tools/m4-xpack/releases`}>GitHub Releases</a> pages.
...
<ul>
<li><code>xpack-m4-{customField('xpackVersion')}-darwin-x64.tar.gz</code></li>
<li><code>xpack-m4-{customField('xpackVersion')}-darwin-arm64.tar.gz</code></li>
</ul>
...
To manually install the <b>xPack GNU M4</b>,
unpack the archive and move it to a location of your choice.

The recommended location is
the <code>~/Library/xPacks/m4</code> folder, for
example <code>/Users/ilg/Library/xPacks/m4/xpack-m4-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/m4
cd ~/Library/xPacks/m4

tar xvf ~/Downloads/xpack-m4-${customField('xpackVersion')}-darwin-x64.tar.gz
chmod -R -w xpack-m4-${customField('xpackVersion')}
`} </CodeBlock>
...
<ul>
<li><code>xpack-m4-{customField('xpackVersion')}-linux-x64.tar.gz</code></li>
<li><code>xpack-m4-{customField('xpackVersion')}-linux-arm64.tar.gz</code></li>
<li><code>xpack-m4-{customField('xpackVersion')}-linux-arm.tar.gz</code></li>
</ul>
...
The recommended location is
the <code>~/.local/xPacks/m4</code> folder, for
example <code>/home/ilg/.local/xPacks/m4/xpack-m4-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/m4
cd ~/.local/xPacks/m4

tar xvf ~/Downloads/xpack-m4-${customField('xpackVersion')}-linux-x64.tar.gz
chmod -R -w xpack-m4-${customField('xpackVersion')}
`} </CodeBlock>
