...
The easiest (and recommended) way to install xPack GNU M4
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/m4@1.4.19-3.1 --verbose
`} </CodeBlock>
...
* add <code>@xpack-dev-tools/m4</code> to
  `package.json` as a
  development dependency; this associates a specific version of xPack GNU M4 with
...
it is also possible to install xPack GNU M4 only globally in
...
In order to access the xPack GNU M4 binaries installed in
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/m4/1.4.19-3.1/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/m4/1.4.19-3.1/.content/bin:$PATH
`} </CodeBlock>
...
<ul>
<li><code>xpack-m4-1.4.19-3-darwin-x64.tar.gz</code></li>
<li><code>xpack-m4-1.4.19-3-darwin-arm64.tar.gz</code></li>
</ul>
...
example <code>/Users/ilg/Library/xPacks/m4/xpack-m4-1.4.19-3</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/m4
cd ~/Library/xPacks/m4

tar xvf ~/Downloads/xpack-m4-1.4.19-3-darwin-x64.tar.gz
chmod -R -w xpack-m4-1.4.19-3
`} </CodeBlock>
...
<ul>
<li><code>xpack-m4-1.4.19-3-linux-x64.tar.gz</code></li>
<li><code>xpack-m4-1.4.19-3-linux-arm64.tar.gz</code></li>
<li><code>xpack-m4-1.4.19-3-linux-arm.tar.gz</code></li>
</ul>
...
example <code>/home/ilg/.local/xPacks/m4/xpack-m4-1.4.19-3</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/m4
cd ~/.local/xPacks/m4

tar xvf ~/Downloads/xpack-m4-1.4.19-3-linux-x64.tar.gz
chmod -R -w xpack-m4-1.4.19-3
`} </CodeBlock>
